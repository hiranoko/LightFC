...
def generate_bbox_mask(bbox_mask, bbox):
    ...
    for i in range(b):
        ...
        bbox_mask[
            i,
            int(bbox_i[1]) : int(bbox_i[1] + bbox_i[3] - 1),
            int(bbox_i[0]) : int(bbox_i[0] + bbox_i[2] - 1),
        ] = 1
    return bbox_mask
...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == "ALL":
        ...
        box_mask_z = torch.zeros(
            [bs, template_feat_size, template_feat_size], device=device
        )
        ...
        box_mask_z = (
            generate_bbox_mask(box_mask_z, gt_bbox * template_size)
            .unsqueeze(1)
            .to(torch.float)
        )  # (batch, 1, 128, 128)
        ...
        box_mask_z = F.interpolate(
            box_mask_z,
            scale_factor=1.0 / cfg.MODEL.BACKBONE.STRIDE,
            mode="bilinear",
            align_corners=False,
        )
        box_mask_z = box_mask_z.flatten(1).to(torch.bool)
...
def adjust_keep_rate(
    ...
    keep_rate = (
        base_keep_rate
        + (max_keep_rate - base_keep_rate)
        * (math.cos(iters / total_iters * math.pi) + 1)
        * 0.5
    )
