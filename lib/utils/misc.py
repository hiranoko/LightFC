...
def _max_by_axis(the_list):
    ...
    for sublist in the_list[1:]:  # [h,w,3]
        for index, item in enumerate(sublist):  # index: 0,1,2
            maxes[index] = max(
                maxes[index], item
            )  # compare current max with the other elements in the whole
    return maxes
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 3:
        ...
        for img, pad_img, m in zip(tensor_list, tensor, mask):
            pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(
                img
            )  # copy valid regions of the images to the largest padded base.
