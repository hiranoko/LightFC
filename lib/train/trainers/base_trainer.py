...
class BaseTrainer:
        ...
        if self.settings.env.workspace_dir is not None:
            ...
            if self.settings.local_rank in [-1, 0]:
                if not os.path.exists(self._checkpoint_dir):
                    print(
                        "Training with multiple GPUs. checkpoints directory doesn't exist. "
        ...
        for i in range(num_tries):
            try:
                ...
                if load_previous_ckpt:
                    directory = "{}/{}".format(
                        self._checkpoint_dir, self.settings.project_path_prv
                    )
                    self.load_state_dict(directory)
                if distill:
                    directory_teacher = "{}/{}".format(
                        self._checkpoint_dir, self.settings.project_path_teacher
                    )
                ...
                for epoch in range(self.epoch + 1, max_epochs + 1):
                    ...
                    # use swa
                    if self.settings.use_swa:
                        if swa_init and epoch == self.settings.swa_epoch:
                            self.net_swa = torch.optim.swa_utils.AveragedModel(
                                self.actor.net
                            )
                            swa_init = False
                            print(f"\n SWA init finished in epoch{self.epoch}! \n")
                        if epoch > self.settings.swa_epoch:
                            self.net_swa.update_parameters(self.actor.net)
                            ema_param = self.net_swa.module.state_dict()
                            self.actor.net.load_state_dict(ema_param)
                            print(
                                f"\nUsing SWA and checkpoint is fused in epoch {self.epoch}! \n"
                            )
                    ...
                    if (
                        epoch > (max_epochs - 1)
                        or save_every_epoch
                        or epoch % self.settings.save_interval == 0
                        or epoch % 40 == 0
                        or epoch in save_epochs
                        or epoch > (max_epochs - 5)
                    ):
    ...
    def load_checkpoint(
        ...
        assert (
            net_type == checkpoint_dict["net_type"]
        ...
        if (
            load_constructor
            and "constructor" in checkpoint_dict
