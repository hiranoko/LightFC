...
class SiamCARLossComputation(object):
    ...
    def compute_targets_for_locations(self, locations, labels, gt_bbox):
        ...
        s1 = (
            reg_targets_per_im[:, :, 0]
            > 0.6 * ((bboxes[:, 2] - bboxes[:, 0]) / 2).float()
        )
        s2 = (
            reg_targets_per_im[:, :, 2]
            > 0.6 * ((bboxes[:, 2] - bboxes[:, 0]) / 2).float()
        )
        s3 = (
            reg_targets_per_im[:, :, 1]
            > 0.6 * ((bboxes[:, 3] - bboxes[:, 1]) / 2).float()
        )
        s4 = (
            reg_targets_per_im[:, :, 3]
            > 0.6 * ((bboxes[:, 3] - bboxes[:, 1]) / 2).float()
        )
        ...
        return (
            labels.permute(1, 0).contiguous(),
            reg_targets_per_im.permute(1, 0, 2).contiguous(),
        )
    ...
    def __call__(
        self, locations, box_cls, box_regression, centerness, labels, reg_targets
    ):
        """
        Arguments:
        ...
        box_regression_flatten = (
            box_regression.permute(0, 2, 3, 1).contiguous().view(-1, 4)
        )
