...
def bbox_iou(
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if (
            CIoU or DIoU or EIoU or SIoU or WIoU
            ...
            if (
                ...
                return (
                    iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2),
                    return_iou,
                )  # EIou
            elif SIoU:
                # SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
                s_cw, s_ch = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5 + eps, (
                    b2_y1 + b2_y2 - b1_y1 - b1_y2
                ) * 0.5 + eps
                sigma = torch.pow(s_cw**2 + s_ch**2, 0.5)
                sin_alpha_1, sin_alpha_2 = (
                ...
                omiga_w, omiga_h = torch.abs(w1 - w2) / torch.max(w1, w2), torch.abs(
                    h1 - h2
                ) / torch.max(h1, h2)
                ...
                if Focal:
                    return iou - torch.pow(
                        0.5 * (distance_cost + shape_cost) + eps, alpha
                    ), torch.pow(
                        inter / (union + eps), gamma
                    )  # Focal_SIou
                return (
                    iou - torch.pow(0.5 * (distance_cost + shape_cost) + eps, alpha),
                    return_iou,
                )  # SIou
            elif WIoU:
                if scale:
                    return (
                        getattr(WIoU_Scale, "_scaled_loss")(wise_scale),
                        (1 - iou) * torch.exp((rho2 / c2)),
                        iou,
                    )  # WIoU v3 https://arxiv.org/abs/2301.10051
                return iou, torch.exp((rho2 / c2)), return_iou  # WIoU v1
            if Focal:
                return iou - rho2 / c2, torch.pow(
                    inter / (union + eps), gamma
                )  # Focal_DIoU
        ...
        return (
            iou - torch.pow((c_area - union) / c_area + eps, alpha),
            return_iou,
