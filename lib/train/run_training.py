...
def run_training(
    ...
    settings = ws_settings.Settings(env_num)
    settings.env_num = env_num
    ...
    if script_name_prv is not None and config_name_prv is not None:
        settings.project_path_prv = "train/{}/{}".format(
            script_name_prv, config_name_prv
        )
    settings.local_rank = local_rank
    settings.save_dir = os.path.abspath(save_dir)
    settings.use_lmdb = use_lmdb
    prj_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
    settings.cfg_file = os.path.join(
        prj_dir, "experiments/%s/%s.yaml" % (script_name, config_name)
    )
    ...
    if distill:
        ...
        if script_teacher is not None and config_teacher is not None:
            settings.project_path_teacher = "train/{}/{}".format(
                script_teacher, config_teacher
            )
        settings.cfg_file_teacher = os.path.join(
            prj_dir, "experiments/%s/%s.yaml" % (script_teacher, config_teacher)
        )
...
def main():
    ...
    parser.add_argument(
        "--script", type=str, required=True, help="Name of the train script."
    )
    parser.add_argument(
        "--config", type=str, required=True, help="Name of the config file."
    )
    parser.add_argument(
        "--cudnn_benchmark",
        type=bool,
    ...
    parser.add_argument(
        "--save_dir", type=str, help="the directory to save checkpoints and logs"
    )
    parser.add_argument("--seed", type=int, default=42, help="seed for random numbers")
    parser.add_argument(
        "--use_lmdb", type=int, choices=[0, 1], default=0
    )  # whether datasets are in lmdb format
    parser.add_argument(
        "--script_prv",
        type=str,
        default=None,
        help="Name of the train script of previous model.",
    )
    parser.add_argument(
        "--config_prv",
        type=str,
        default=None,
        help="Name of the config file of previous model.",
    )
    parser.add_argument(
        "--use_wandb", type=int, choices=[0, 1], default=0
    ...
    parser.add_argument(
        "--distill", type=int, choices=[0, 1], default=0
    )  # whether to use knowledge distillation
    parser.add_argument("--script_teacher", type=str, help="teacher script name")
    parser.add_argument(
        "--config_teacher", type=str, help="teacher yaml configure file name"
    )
