...
class lightTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        gt_gaussian_maps = generate_heatmap(
            gt_dict["search_anno"].view(n, bs, 4),
        ...
        gt_boxes_vec = (
            box_xywh_to_xyxy(gt_bbox).view(-1, 4).clamp(min=0.0, max=1.0)
        )  # (B,4) --> (B,1,4) --> (B,N,4)

        # locate box
        try:
            iou_loss, iou = self.objective.iou(
        ...
        if "score_map" in pred_dict:
            location_loss = self.objective.focal_loss(
