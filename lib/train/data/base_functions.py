    ...
    for name in name_list:
        ...
        if name == "VID":
            if settings.use_lmdb:
                ...
                datasets.append(
                    ImagenetVID_lmdb(
                        settings.env.imagenet_lmdb_dir,
                        image_loader=image_loader,
                        env_num=settings.env_num,
                    )
                )
        ...
        if name == "TRACKINGNET":
            if settings.use_lmdb:
                ...
                datasets.append(
                    TrackingNet_lmdb(
                        settings.env.trackingnet_lmdb_dir,
                        image_loader=image_loader,
                        env_num=settings.env_num,
                    )
                )
...
def build_dataloaders(cfg, settings):
    ...
    train_sampler = (
        DistributedSampler(dataset_train) if settings.local_rank != -1 else None
    )
...
def get_optimizer_scheduler(net, cfg):
    ...
    if cfg.TRAIN.OPTIMIZER == "ADAMW":
        ...
        optimizer = torch.optim.SGD(
            param_dicts, lr=cfg.TRAIN.LR, momentum=cfg.TRAIN.MOMENTUM
        )
    else:
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == "step":
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=cfg.TRAIN.SCHEDULER.COS_CYCLE, eta_min=0.0001
        )
    else:
