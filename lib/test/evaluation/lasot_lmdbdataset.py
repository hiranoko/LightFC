...
class LaSOTlmdbDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        occlusion_label_path = str(
            "{}/{}/full_occlusion.txt".format(class_name, sequence_name)
        )
        occ_list = list(
            map(int, decode_str(self.base_path, occlusion_label_path).split(","))
        )
        full_occlusion = np.array(occ_list).astype(np.float64)
        # decode out of view file
        out_of_view_label_path = str(
            "{}/{}/out_of_view.txt".format(class_name, sequence_name)
        )
        out_of_view_list = list(
            map(int, decode_str(self.base_path, out_of_view_label_path).split(","))
        )
        ...
        frames_list = [
            [self.base_path, "{}/{:08d}.jpg".format(frames_path, frame_number)]
