...
class Tracker:
    ...
    def __init__(
        ...
        assert env_num is None or isinstance(env_num, int)
        ...
        self.deploy = deploy
        # if name.split('_')[-1] == 'onnx':
        #     self.deploy = True

        env = env_settings(env_num)
        ...
        if self.deploy:
            ...
            with open(os.devnull, "w") as f:
                sys.stdout = f
                params = self.get_parameters(env_num=env_num)
    ...
    def run_sequence(self, seq, debug=None, env_num=0):
        ...
        if self.deploy:
            self.tracker.state = None
            self.tracker.frame_id = 0
            output = self._track_sequence(self.tracker, seq, init_info)
        else:
    ...
    def get_parameters(self, env_num=0):
        """Get parameters."""
        param_module = importlib.import_module(
            "lib.test.parameter.{}".format(self.name)
        )
        params = param_module.parameters(self.parameter_name, env_num=env_num)
        return params
