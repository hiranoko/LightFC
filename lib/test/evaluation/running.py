...
def run_sequence(
    seq: Sequence, tracker: Tracker, debug=False, num_gpu=8, env_num=0, len_dataset=0
):
...
def run_dataset(
    dataset, trackers, debug=False, threads=0, num_gpus=8, env_num=0, deploy=True
):
    ...
    print(
        f"Evaluating {trackers[0].name} {trackers[0].parameter_name} on {len(dataset):5d} sequences"
    )
    ...
    if mode == "sequential":
        ...
        param_list = [
            (seq, tracker_info, debug, num_gpus, env_num, len(dataset))
    ...
    print(
        "Done, total time: {}".format(
            str(timedelta(seconds=(time.time() - dataset_start_time)))
        ),
        "\n",
    )
