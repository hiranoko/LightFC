...
def check_eval_data_is_valid(eval_data, trackers, dataset):
    ...
    tracker_names_f_saved = [
        (t["name"], t["param"], t["run_id"]) for t in eval_data["trackers"]
    ]
...
def merge_multiple_runs(eval_data):
    ...
    for i in range(len(trackers)):
        ...
        match = [
            t["name"] == base_tracker["name"] and t["param"] == base_tracker["param"]
            for t in trackers
        ]
        match = torch.tensor(match)

        ave_success_rate_plot_overlap_merged.append(
            ave_success_rate_plot_overlap[:, match, :].mean(1)
        )
        ave_success_rate_plot_center_merged.append(
            ave_success_rate_plot_center[:, match, :].mean(1)
        )
        ave_success_rate_plot_center_norm_merged.append(
            ave_success_rate_plot_center_norm[:, match, :].mean(1)
        )
        avg_overlap_all_merged.append(avg_overlap_all[:, match].mean(1))

        merged[match] = 1

    ave_success_rate_plot_overlap_merged = torch.stack(
        ave_success_rate_plot_overlap_merged, dim=1
    )
    ave_success_rate_plot_center_merged = torch.stack(
        ave_success_rate_plot_center_merged, dim=1
    )
    ave_success_rate_plot_center_norm_merged = torch.stack(
        ave_success_rate_plot_center_norm_merged, dim=1
    )
    avg_overlap_all_merged = torch.stack(avg_overlap_all_merged, dim=1)

    eval_data["trackers"] = new_tracker_names
    eval_data["ave_success_rate_plot_overlap"] = (
        ave_success_rate_plot_overlap_merged.tolist()
    )
    eval_data["ave_success_rate_plot_center"] = (
        ave_success_rate_plot_center_merged.tolist()
    )
    eval_data["ave_success_rate_plot_center_norm"] = (
        ave_success_rate_plot_center_norm_merged.tolist()
    )
...
def plot_draw_save(
    ...
    fig.savefig(
        "{}/{}_plot.pdf".format(result_plot_path, plot_type),
        dpi=300,
        format="pdf",
        transparent=True,
    )
...
def check_and_load_precomputed_results(
    trackers, dataset, report_name, force_evaluation=False, env_num=None, **kwargs
):
    # Load data
    settings = env_settings(env_num=env_num)
    ...
    if os.path.isfile(eval_data_path) and not force_evaluation:
        ...
        eval_data = extract_results(
            trackers, dataset, report_name, env_num=env_num, **kwargs
        )
...
def plot_results(
    ...
    **kwargs
    ...
    eval_data = check_and_load_precomputed_results(
        trackers, dataset, report_name, force_evaluation, **kwargs
    )
...
def print_results(
    ...
    **kwargs
    ...
    report_text = generate_formatted_report(
        tracker_disp_names, scores, table_name=report_name
    )
    print(report_text)


def plot_got_success(trackers, report_name, env_num):
...
def print_per_sequence_results(
    trackers,
    dataset,
    report_name,
    merge_results=False,
    ...
    **kwargs
    ...
    if filter_criteria is not None:
        if filter_criteria["mode"] == "ao_min":
            ...
            valid_sequence = valid_sequence & (
                (max_ao - min_ao) > filter_criteria["threshold"]
            )
        else:
            raise Exception
    ...
    sequence_names = [
        s + " (ID={})".format(i)
        for i, (s, v) in enumerate(zip(sequence_names, valid_sequence.tolist()))
        if v
    ]
    ...
    scores_per_tracker = {
        k: avg_overlap_all[:, i] for i, k in enumerate(tracker_disp_names)
    }
    ...
    **kwargs
    ...
    if per_video:
        for i in range(seq_lens):
            ...
            # Print
            tracker_disp_names = [
                get_tracker_display_name(trk) for trk in tracker_names
            ]
            report_text = generate_formatted_report(
                tracker_disp_names, scores, table_name=report_name
            )
