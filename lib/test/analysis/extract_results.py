...
def calc_iou_overlap(pred_bb, anno_bb):
    ...
    br = torch.min(
        pred_bb[:, :2] + pred_bb[:, 2:] - 1.0, anno_bb[:, :2] + anno_bb[:, 2:] - 1.0
    )
    ...
    if pred_bb.shape[0] != anno_bb.shape[0]:
        if dataset == "lasot":
            ...
            if pred_bb.shape[0] > anno_bb.shape[0]:
                ...
                pad = torch.zeros((anno_bb.shape[0] - pred_bb.shape[0], 4)).type_as(
                    pred_bb
                )
...
def extract_results(
    ...
    ave_success_rate_plot_overlap = torch.zeros(
        (len(dataset), len(trackers), threshold_set_overlap.numel()),
        dtype=torch.float32,
    )
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            base_results_path = "{}/{}/{}".format(
                trk.results_dir, report_name, seq.name
            )
            ...
            if os.path.isfile(results_path):
                pred_bb = torch.tensor(
                    load_text(
                        str(results_path), delimiter=("\t", ","), dtype=np.float64
                    )
                )
            else:
                if skip_missing_seq:
            ...
            # Calculate measures
            err_overlap, err_center, err_center_normalized, valid_frame = (
            ...
            ave_success_rate_plot_center_norm[seq_id, trk_id, :] = (
                err_center_normalized.view(-1, 1)
                <= threshold_set_center_norm.view(1, -1)
            ).sum(0).float() / seq_length
