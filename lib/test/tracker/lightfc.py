...
from lib.utils.box_ops import box_iou, box_xywh_to_xyxy, box_xyxy_to_xywh, clip_box
...
class lightFC(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            x_dict = search
            out_dict = self.network.forward_tracking(
                z_feat=self.z_feat, x=x_dict.tensors
            )

        response_origin = self.output_window * out_dict["score_map"]

        pred_box_origin = self.compute_box(
            response_origin, out_dict, resize_factor
        ).tolist()  # .unsqueeze(dim=0)  # tolist()

        self.state = clip_box(
            self.map_box_back(pred_box_origin, resize_factor), H, W, margin=2
        )
